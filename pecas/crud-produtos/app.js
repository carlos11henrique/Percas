...
async function diminuirQuantidadeProduto(idProduto, quantidadeComprada) {
    try {
        const productRef = doc(db, 'produtos', idProduto);
        const productSnap = await getDoc(productRef);
        
        if (productSnap.exists()) {
            const { quantidade } = productSnap.data();
            const novaQuantidade = quantidade - quantidadeComprada;
...
const idProdutoComprado = 'ID_DO_PRODUTO_AQUI';
const quantidadeComprada = 1; // Por exemplo, se uma unidade do produto foi comprada

diminuirQuantidadeProduto(idProdutoComprado, quantidadeComprada);
